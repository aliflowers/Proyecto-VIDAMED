...
function buildAppointmentEmailHtml(opts: {
  ...
  const heading = opts.type === 'confirmation' ? 'Confirmación de Cita' : 'Recordatorio de Cita'
  ...
  return `<!doctype html><html><head><meta charset="utf-8" /></head><body style="margin:0;padding:0;background:${C.bg};"><table role="presentation" cellspacing="0" cellpadding="0" width="100%" style="border-collapse:collapse;"><tr><td align="center" style="padding:24px;"><table role="presentation" width="600" style="max-width:600px;border-collapse:collapse;"><tr><td style="background:#ffffff;border-radius:12px 12px 0 0;padding:24px;">${logoImg}</td></tr><tr><td style="background:#ffffff;border-radius:0 0 12px 12px;padding:0 24px 24px;"><h1 style="margin:8px 0 0;font-family:Segoe UI, Arial, sans-serif;font-size:22px;line-height:28px;color:${C.text};">${heading} – ${BRAND.name}</h1><p style="margin:4px 0 16px;color:${C.muted};font-size:12px;font-family:Segoe UI, Arial, sans-serif;">${opts.dateStr} • ${opts.timeStr}</p><p style="font-size:14px;color:${C.text};margin:0 0 8px;font-family:Segoe UI, Arial, sans-serif;">Hola <strong>${opts.patientName}</strong>,</p><p style="font-size:14px;color:${C.text};margin:0 0 16px;font-family:Segoe UI, Arial, sans-serif;">${intro}</p><div style="margin:12px 0;padding:12px 16px;border:1px solid ${C.border};border-radius:8px;background:#F9FAFB;"><p style="font-size:14px;color:${C.text};margin:0 0 8px;font-family:Segoe UI, Arial, sans-serif;">Ubicación: <strong>${opts.location}</strong></p>${phoneLine}${cedulaLine}${studiesHtml}</div>${summaryText}${reminderBlock}<p style="font-size:12px;color:${C.muted};margin-top:16px;font-family:Segoe UI, Arial, sans-serif;">Mensaje automático ${BRAND.name}. Si necesitas ayuda, contáctanos.</p></td></tr><tr><td style="text-align:center;padding:16px;color:${C.muted};font-size:12px;font-family:Segoe UI, Arial, sans-serif;">© ${new Date().getFullYear()} ${BRAND.name} — Todos los derechos reservados</td></tr></table></td></tr></table></body></html>`
}
...
function getSmtpConfig() {
  ...
  if (!host || !port || !user || !pass) throw new Error('Config SMTP incompleta: SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS.')
...
export async function sendAppointmentConfirmationEmail(opts: {
  ...
  const html = buildAppointmentEmailHtml({ type: 'confirmation', patientName: opts.patientName, dateStr, timeStr, location: opts.location, studies: opts.studies || [], phone: opts.phone, cedula: opts.cedula, summaryText: opts.summaryText, logoCid, whatsappNumber: getEnv('WHATSAPP_NUMBER') || undefined })
  const subject = `VidaMed – Confirmación de Cita – ${dateStr} ${timeStr}`
  const info = await transporter.sendMail({ to: opts.to, from: (getEnv('MAIL_CONFIRMATION_FROM') || getEnv('EMAIL_CONFIRMATION_FROM') || 'VidaMed <atencionalpaciente@labvidamed.com>'), replyTo: smtp.replyTo, subject, html, attachments: hasLogo ? [{ filename: 'vidamed_logo.png', path: logoPath, cid: logoCid }] : [] })
...
export async function sendAppointmentReminderEmail(opts: {
  ...
  const logoCid = hasLogo ? 'vidamed-logo' : undefined
  const { dateStr, timeStr } = formatDateVZ(opts.dateIso)
  const html = buildAppointmentEmailHtml({ type: 'reminder', patientName: opts.patientName, dateStr, timeStr, location: opts.location, studies: opts.studies || [], phone: opts.phone, cedula: opts.cedula, logoCid, whatsappNumber: getEnv('WHATSAPP_NUMBER') || undefined })
  const subject = `VidaMed – Recordatorio de Cita – ${dateStr} ${timeStr}`
  const info = await transporter.sendMail({ to: opts.to, from: smtp.from, replyTo: smtp.replyTo, subject, html, attachments: hasLogo ? [{ filename: 'vidamed_logo.png', path: logoPath, cid: logoCid }] : [] })
  return { ok: true, messageId: info.messageId }
}
