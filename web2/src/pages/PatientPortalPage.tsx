...
import { User, Search, Eye, Loader } from 'lucide-react';
                ...
                <div className="w-full max-w-2xl mx-auto p-8 space-y-8 bg-white rounded-xl shadow-lg">
                    ...
                    <form className="mt-8 space-y-6" onSubmit={handleSearch}>
                        ...
                        <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                            {isLoading ? <Loader className="animate-spin" /> : <><Search className="mr-2" />Buscar Resultados</>}
                        </button>
                    </form>
                    ...
                    {searched && !isLoading && !errorMessage && !patient && (
                    ...
                    {patient && testimonialStatus !== 'submitted' && (
                        <TestimonialForm 
                            studies={results}
